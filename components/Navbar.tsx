...
import { PlusCircle, User, Settings, Building2, CircleUser, Menu } from 'lucide-react'
...
export function Navbar() {
  ...
  return (
    <header className={`w-full bg-white relative z-50 shadow-[0_1px_12px_rgba(0,0,0,0.08)] ${hideOnMobile}`}>
      <div className='flex justify-between items-center px-6 md:px-10 h-20 border-b border-outline-variant/10'>
        {/* Logo */}
        <Link href='/' className='flex items-center gap-2 md:gap-3'>
          <Building2 className='w-8 h-8 text-[#FF385C] fill-[#FF385C]/10 shrink-0' strokeWidth={2} />
        ...
        <nav className='hidden md:flex items-center gap-8'>
          <Link href='/' className={`text-sm font-semibold transition-colors ${pathname === '/' ? 'text-on-surface' : 'text-on-surface-variant hover:text-on-surface'}`}>Explore</Link>
          <Link href='/saved' className={`text-sm font-semibold transition-colors ${pathname === '/saved' ? 'text-on-surface' : 'text-on-surface-variant hover:text-on-surface'}`}>Saved</Link>
          <Link href='/bookings' className={`text-sm font-semibold transition-colors ${pathname === '/bookings' ? 'text-on-surface' : 'text-on-surface-variant hover:text-on-surface'}`}>Bookings</Link>
        </nav>
        ...
        <div className='flex items-center gap-2 relative'>
          ...
          >
            <Menu className='w-4 h-4 text-on-surface-variant ml-1' strokeWidth={2.5} />
          ...
          {showDropdown && (
            <div className='absolute top-14 right-0 bg-white border border-outline-variant/30 shadow-[0_8px_30px_rgba(0,0,0,0.15)] rounded-2xl p-2 w-56 animate-in fade-in zoom-in-95 duration-200 z-[60]'>
              <div className='flex flex-col'>
                ...
                >
                  <PlusCircle className='w-5 h-5 text-[#FF385C]' strokeWidth={2.5} />
                  Become a owner
                </Link>
