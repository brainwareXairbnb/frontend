  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.error('Restoration Failed', {
        description: err.message || 'OTP may have expired',
      })
    } finally {
      setLoading(false)
    }
    ...
    <div className='min-h-screen flex flex-col md:flex-row overflow-hidden bg-white'>
      ...
      <div className='flex-1 flex flex-col items-center md:justify-center p-6 md:p-10 lg:p-14 bg-white overflow-y-auto'>
        <header className='w-full max-w-md mb-6 md:mb-10 animate-in fade-in slide-in-from-top-4 duration-700'>
          ...
          <h2 className='text-5xl font-headline font-black text-on-surface tracking-tighter mb-2 uppercase leading-none'>
        ...
        <main className='w-full max-w-md animate-in fade-in slide-in-from-bottom-4 duration-700 delay-100'>
          <form onSubmit={handleSubmit} className='space-y-4 md:space-y-5'>
            ...
            <div className='space-y-2'>
              ...
              <div className='relative group/input'>
                <input
                  className='w-full h-14 bg-[#FAFAFA] border-2 border-outline-variant/5 rounded-[1.2rem] px-6 text-2xl font-black text-on-surface focus:border-primary outline-none transition-all tracking-[0.5em] text-center'
                  type='text'
                  placeholder='000000'
                  maxLength={6}
                  value={otp}
