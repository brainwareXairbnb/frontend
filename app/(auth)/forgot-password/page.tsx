...
export default function ForgotPasswordPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.success('Restoration Initialized', { description: 'Success sequence dispatched' });
      setSuccess(true);
    } catch (err: any) {
      toast.error('Initialization Failed', { description: err.message || 'Please try again.' });
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="min-h-screen flex items-center justify-center p-4 bg-[#F8F9FA] relative overflow-hidden font-sans">
        ...
        <div className="w-full max-w-md bg-white border border-outline-variant/10 rounded-[2.5rem] p-10 md:p-14 shadow-[0_32px_64px_-16px_rgba(0,0,0,0.08)] text-center animate-in fade-in zoom-in duration-1000 relative z-10">
          ...
          <div className="space-y-2 mb-8">
            <h2 className="text-[10px] font-black uppercase tracking-[0.4em] text-emerald-600 mb-2">Transmission Successful</h2>
            <h1 className="text-3xl font-headline font-black text-on-surface tracking-tighter uppercase leading-none">
              Sequence <br />
              <span className="text-primary/40">Dispatched</span>
            </h1>
          </div>
          ...
          <p className="text-[11px] font-medium text-on-surface-variant leading-relaxed opacity-60 mb-12 max-w-[280px] mx-auto uppercase tracking-widest">
            A secure recovery identifier has been transmitted to <span className="text-on-surface opacity-100 font-bold">{email}</span>.
          </p>
    ...
    <div className="min-h-screen flex flex-col md:flex-row overflow-hidden bg-white">
      ...
      <div className="hidden md:flex md:w-1/2 lg:w-3/5 relative bg-on-surface overflow-hidden">
        ...
        <div className="relative z-20 mt-auto p-20 max-w-2xl animate-in slide-in-from-left duration-1000">
          ...
          <p className="text-xl font-medium text-surface/60 leading-relaxed max-w-md uppercase tracking-widest text-[12px]">
            We'll transmit a secure encrypted link to recalibrate your account credentials and restore access.
          </p>
        </div>
      </div>
      ...
      <div className="flex-1 flex flex-col items-center md:justify-center p-6 md:p-16 lg:p-24 bg-white overflow-y-auto">
        <header className="w-full max-w-md mb-12 md:mb-20 animate-in fade-in slide-in-from-top-4 duration-1000">
          <Link href="/" className="flex items-center gap-3 mb-12 md:mb-20 group">
            ...
            <div className="flex flex-col">
              <span className="font-headline font-black text-3xl tracking-tighter text-on-surface uppercase leading-none">
                Brainware <span className="text-primary/40 text-2xl">Rooms</span>
              </span>
              <span className="text-[8px] font-black text-primary uppercase tracking-[0.5em] mt-1 opacity-60">Identity Service</span>
            </div>
          </Link>
          ...
          <form onSubmit={handleSubmit} className="space-y-6 md:space-y-10">
            <div className="space-y-4">
              ...
              <div className="relative group/input">
                ...
                <input
                  className="w-full h-20 bg-[#FAFAFA] border-2 border-outline-variant/10 rounded-[1.5rem] pl-16 pr-6 text-sm font-black text-on-surface focus:border-primary focus:bg-white focus:shadow-xl focus:shadow-primary/5 outline-none transition-all duration-300 placeholder:opacity-30 uppercase tracking-widest"
                  id="email"
                  type="email"
            ...
            <Button
              ...
              disabled={loading}
            >
              {loading ? <Loader2 className="w-4 h-4 animate-spin" /> : 'Initialize Restoration'}
        ...
        <footer className="w-full max-w-md mt-12 text-center border-t border-outline-variant/5 pt-8 md:pt-12">
