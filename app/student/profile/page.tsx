...
function DesktopProfileMenuRow({
  ...
  return (
    <button
      onClick={onClick}
      className='w-full flex items-center gap-4 py-4 px-3 hover:bg-slate-50/70 rounded-xl text-left cursor-pointer transition-all group '
    >
      ...
      ? [
        {
          ...
          description: 'Earn money by renting out your student space',
          icon: Building2,
          path: '/host/homes',
        },
      ]
        ...
        <div className='container mx-auto px-4 sm:px-6 lg:px-8 pt-6 pb-24'>
          ...
          <div className='bg-white rounded-2xl border border-slate-100 shadow-sm p-6 mb-8 flex flex-col sm:flex-row sm:items-center justify-between gap-4'>
            <div className='flex items-center gap-5'>
              <div className='relative shrink-0'>
                <div className='w-16 h-16 sm:w-20 sm:h-20 rounded-full bg-gradient-to-br from-[#FF385C] to-[#E31C5F] flex items-center justify-center overflow-hidden'>
                  {user?.profilePicUrl ? (
                    <Image
                      src={user.profilePicUrl}
                      alt={user?.name || 'Profile'}
          ...
          {!user.isStudentVerified && user.role === 'student' && (
            <button
              onClick={() => router.push('/student/verify-student-email')}
              className='w-full flex items-center gap-3 p-4 mb-6 rounded-xl text-left border border-rose-100 hover:bg-rose-50/70 transition-all cursor-pointer'
